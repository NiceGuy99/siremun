...
export default function Index({ records, nextPenjaminId, filters }) {
    ...
    const handleFilter = (e) => {
        e?.preventDefault();
        router.get(
            route('admin.master-penjamin.index'),
            { search },
    ...
    const handleReset = () => {
        setSearch('');
        router.get(route('admin.master-penjamin.index'), {}, { preserveState: true, preserveScroll: true });
    };

    // ── CRUD handlers ──
    const resetForm = () => {
        setFormData({ PENJAMIN_ID: nextPenjaminId || '', NAMA_PENJAMIN: '', STATUS: 1 });
        setFormErrors({});
    };
    ...
    const openEdit = (record) => {
        ...
        setFormData({
            ...
            NAMA_PENJAMIN: record.NAMA_PENJAMIN || '',
    ...
    const handleStore = (e) => {
        ...
        router.post(route('admin.master-penjamin.store'), formData, {
            preserveScroll: true,
    ...
    const handleUpdate = (e) => {
        e.preventDefault();
        setFormProcessing(true);
        router.put(route('admin.master-penjamin.update', editingRecord.ID), formData, {
            preserveScroll: true,
    ...
    const handleDestroy = () => {
        setFormProcessing(true);
        router.delete(route('admin.master-penjamin.destroy', deletingRecord.ID), {
            preserveScroll: true,
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-gray-600 dark:text-gray-400">
                    Penjamin ID <span className="text-red-500">*</span>
                </label>
                <input
                    type="number"
                    value={formData.PENJAMIN_ID}
                    onChange={(e) => setFormData({ ...formData, PENJAMIN_ID: e.target.value })}
                    className={`${inputClass} bg-gray-100 dark:bg-gray-900 cursor-not-allowed`}
                    required
                    disabled
                    placeholder="Contoh: 1"
                />
                {formErrors.PENJAMIN_ID && <span className="text-xs text-red-500 font-semibold">{formErrors.PENJAMIN_ID}</span>}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
                ...
                <input
                    ...
                    onChange={(e) => setFormData({ ...formData, NAMA_PENJAMIN: e.target.value })}
                    className={inputClass}
                    required
                    placeholder="Masukkan nama penjamin"
                />
                {formErrors.NAMA_PENJAMIN && <span className="text-xs text-red-500 font-semibold">{formErrors.NAMA_PENJAMIN}</span>}
            </div>
    ...
    return (
        <AdminLayout title="Master Penjamin">
            <Head title="Master Penjamin - Admin SIREMUN" />
                        ...
                        <div className="flex flex-col gap-1.5 w-full sm:flex-1">
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    placeholder="Cari by Penjamin ID atau Nama Penjamin..."
                        ...
                        <table className="w-full text-left border-collapse text-sm">
                            <thead>
                                <tr className="bg-gray-50 dark:bg-gray-800/40 text-gray-500 dark:text-gray-400 font-bold border-b border-gray-200 dark:border-gray-800">
                                    ...
                                    <th className="px-5 py-3.5 whitespace-nowrap">Penjamin ID</th>
                                    <th className="px-5 py-3.5 whitespace-nowrap">Nama Penjamin</th>
                                    ...
                                    {paginatedRecords && paginatedRecords.length > 0 ? (
                                        paginatedRecords.map((row, idx) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-5 py-3 font-mono text-xs font-semibold text-amber-600 dark:text-amber-400">{row.PENJAMIN_ID}</td>
                                                <td className="px-5 py-3 font-medium text-gray-900 dark:text-white">{row.NAMA_PENJAMIN}</td>
                                        ...
                                        <tr>
                                            <td colSpan="5" className="px-5 py-16 text-center text-gray-500 dark:text-gray-400">
                                                <div className="flex flex-col items-center justify-center gap-2">
                                                    ...
                                                    <span className="text-sm">Tidak ada data Master Penjamin yang ditemukan.</span>
                                                </div>
                                            </td>
                                        </tr>
                                    )}
            ...
            {/* ── Create Modal ── */}
            <Modal show={showCreateModal} onClose={() => setShowCreateModal(false)} title="Tambah Master Penjamin">
            ...
            {/* ── Edit Modal ── */}
            <Modal show={showEditModal} onClose={() => setShowEditModal(false)} title="Ubah Master Penjamin">
            ...
            <Modal show={showDeleteModal} onClose={() => setShowDeleteModal(false)} title="Konfirmasi Hapus" maxWidth="max-w-sm">
                <div className="text-center">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">Apakah Anda yakin ingin menghapus penjamin:</p>
                    <p className="text-sm font-bold text-gray-900 dark:text-white mb-6">
                        {deletingRecord?.NAMA_PENJAMIN}
                        <span className="text-xs font-mono text-gray-400 ml-1">(ID: {deletingRecord?.PENJAMIN_ID})</span>
                    </p>
