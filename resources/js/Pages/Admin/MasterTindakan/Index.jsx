...
export default function Index({ records, kelompokOptions, pagination, filters }) {
    const { flash } = usePage().props;
    const [kelompokId, setKelompokId] = useState(filters.kelompok_id || '');
    ...
    const handleFilter = (e) => {
        e?.preventDefault();
        router.get(
            route('admin.master-tindakan.index'),
            { kelompok_id: kelompokId, search: search, page: 1 },
    ...
    const handleReset = () => {
        setKelompokId('');
        setSearch('');
        router.get(route('admin.master-tindakan.index'), {}, { preserveState: true, preserveScroll: true });
    };
    ...
    const handlePageChange = (page) => {
        ...
        router.get(
            route('admin.master-tindakan.index'),
            { kelompok_id: kelompokId, search: search, page },
            { preserveState: true, preserveScroll: true }
        );
    };
    ...
    // ── CRUD handlers ──
    const resetForm = () => {
        setFormData({ TINDAKAN_ID: '', NAMA_TINDAKAN: '', JENIS: '', FLAG_KELOMPOK: '', STATUS: 1 });
        setFormErrors({});
    };
    ...
    const openEdit = (record) => {
        ...
        setFormData({
            TINDAKAN_ID: record.TINDAKAN_ID || '',
            NAMA_TINDAKAN: record.NAMA_TINDAKAN || '',
            JENIS: record.JENIS !== null && record.JENIS !== undefined ? record.JENIS : '',
            FLAG_KELOMPOK: record.FLAG_KELOMPOK || '',
    ...
    const handleStore = (e) => {
        ...
        router.post(route('admin.master-tindakan.store'), formData, {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleUpdate = (e) => {
        e.preventDefault();
        setFormProcessing(true);
        router.put(route('admin.master-tindakan.update', editingRecord.ID), formData, {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleDestroy = () => {
        setFormProcessing(true);
        router.delete(route('admin.master-tindakan.destroy', deletingRecord.ID), {
            preserveScroll: true,
            onSuccess: () => {
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-gray-600 dark:text-gray-400">
                    Tindakan ID <span className="text-red-500">*</span>
                </label>
                <input
                    type="number"
                    value={formData.TINDAKAN_ID}
                    onChange={(e) => setFormData({ ...formData, TINDAKAN_ID: e.target.value })}
                    className={inputClass}
                    required
                    placeholder="Contoh: 10003"
                />
                {formErrors.TINDAKAN_ID && <span className="text-xs text-red-500 font-semibold">{formErrors.TINDAKAN_ID}</span>}
            </div>

            {/* NAMA_TINDAKAN */}
            <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-gray-600 dark:text-gray-400">
                    Nama Tindakan <span className="text-red-500">*</span>
                </label>
                <input
                    type="text"
                ...
                />
                {formErrors.NAMA_TINDAKAN && <span className="text-xs text-red-500 font-semibold">{formErrors.NAMA_TINDAKAN}</span>}
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {/* JENIS */}
                <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold text-gray-600 dark:text-gray-400">Jenis</label>
                    <input
                        type="number"
                    ...
                    />
                    {formErrors.JENIS && <span className="text-xs text-red-500 font-semibold">{formErrors.JENIS}</span>}
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                    ...
                    <select
                        value={formData.FLAG_KELOMPOK}
                        onChange={(e) => setFormData({ ...formData, FLAG_KELOMPOK: e.target.value })}
                        className={inputClass}
                        required
                    >
                    ...
                    {formErrors.FLAG_KELOMPOK && <span className="text-xs text-red-500 font-semibold">{formErrors.FLAG_KELOMPOK}</span>}
                </div>
            </div>
    ...
    return (
        <AdminLayout title="Master Tindakan">
            <Head title="Master Tindakan - Admin SIREMUN" />
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 shadow-sm">
                    <form onSubmit={handleFilter} className="flex flex-col lg:flex-row items-end gap-4">
                        {/* Kelompok Filter */}
                        <div className="flex flex-col gap-1.5 w-full lg:w-56">
                            <label className="text-xs font-semibold text-gray-600 dark:text-gray-400">Kelompok</label>
                            <select
                                value={kelompokId}
                        ...
                        <div className="flex flex-col gap-1.5 w-full lg:flex-1">
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    placeholder="Cari by Tindakan ID atau Nama Tindakan..."
                        ...
                        <table className="w-full text-left border-collapse text-sm">
                            <thead>
                                <tr className="bg-gray-50 dark:bg-gray-800/40 text-gray-500 dark:text-gray-400 font-bold border-b border-gray-200 dark:border-gray-800">
                                    ...
                                    <th className="px-5 py-3.5 whitespace-nowrap">Tindakan ID</th>
                                    <th className="px-5 py-3.5 whitespace-nowrap">Nama Tindakan</th>
                                    <th className="px-5 py-3.5 whitespace-nowrap">Kelompok</th>
                                    <th className="px-5 py-3.5 whitespace-nowrap w-20 text-center">Jenis</th>
                                    ...
                                    {records && records.length > 0 ? (
                                        records.map((row, idx) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-5 py-3 font-mono text-xs font-semibold text-amber-600 dark:text-amber-400">{row.TINDAKAN_ID}</td>
                                                <td className="px-5 py-3 font-medium text-gray-900 dark:text-white">{row.NAMA_TINDAKAN}</td>
                                                <td className="px-5 py-3">
                                                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-semibold bg-sky-50 text-sky-700 dark:bg-sky-950/20 dark:text-sky-400">
                                                        {row.NAMA_KELOMPOK}
                                                    </span>
                                                </td>
                                                <td className="px-5 py-3 text-center font-mono text-xs">{row.JENIS ?? '-'}</td>
                                        ...
                                        <tr>
                                            <td colSpan="7" className="px-5 py-16 text-center text-gray-500 dark:text-gray-400">
                                                <div className="flex flex-col items-center justify-center gap-2">
                                                    ...
                                                    <span className="text-sm">Tidak ada data Master Tindakan yang ditemukan.</span>
                                                </div>
                                            </td>
                                        </tr>
                                    )}
            ...
            {/* ── Create Modal ── */}
            <Modal show={showCreateModal} onClose={() => setShowCreateModal(false)} title="Tambah Master Tindakan">
                <form onSubmit={handleStore}>
                    ...
                    <div className="flex items-center justify-end gap-3 pt-5 mt-5 border-t border-gray-100 dark:border-gray-800">
                        ...
                        <button
                            ...
                            className="inline-flex items-center justify-center gap-2 rounded-xl text-sm font-semibold py-2.5 px-6 bg-amber-500 hover:bg-amber-600 text-white shadow-md shadow-amber-500/10 transition duration-150 disabled:opacity-50"
                        >
                            {formProcessing ? (
                                <>
                                    <svg className="animate-spin h-4 w-4 text-white flex-shrink-0" fill="none" viewBox="0 0 24 24">
            ...
            {/* ── Edit Modal ── */}
            <Modal show={showEditModal} onClose={() => setShowEditModal(false)} title="Ubah Master Tindakan">
            ...
            <Modal show={showDeleteModal} onClose={() => setShowDeleteModal(false)} title="Konfirmasi Hapus" maxWidth="max-w-sm">
                <div className="text-center">
                    ...
                    <p className="text-sm font-bold text-gray-900 dark:text-white mb-6">
                        {deletingRecord?.NAMA_TINDAKAN}
                        <span className="text-xs font-mono text-gray-400 ml-1">(ID: {deletingRecord?.TINDAKAN_ID})</span>
                    </p>
                    <div className="flex items-center justify-center gap-3">
                        ...
                        <button
                            ...
                            disabled={formProcessing}
                            className="inline-flex items-center justify-center gap-2 rounded-xl text-sm font-semibold py-2.5 px-6 bg-red-500 hover:bg-red-600 text-white shadow-md shadow-red-500/10 transition duration-150 disabled:opacity-50"
                        >
                            {formProcessing ? (
                                <>
                                    <svg className="animate-spin h-4 w-4 text-white flex-shrink-0" fill="none" viewBox="0 0 24 24">
